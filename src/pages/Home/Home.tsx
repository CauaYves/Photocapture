...
import MemberCard, { Member } from "../../components/MemberCard/MemberCard";
import MemberCardSkeleton from "../../components/MemberCard/MemberCardSkeleton";
import { faultMessages } from "../../components/Modal/FaultMessagesModal";
import { successMessages } from "../../components/Modal/SuccessMessagesModal";
import TextCarousel from "../../components/Tutorial/HomeTutorialCarousel";
import { TutorialSlides } from "../../components/Tutorial/TutorialSlides";
import { loadAppSettings } from "../../config/configLoader";
import { cadasterMemberPhoto, initializeApi, listMembers } from "../../services/api";
...
const HomePage: React.FC = () => {
	...
	const { sendingMessage, isTransitioning } = useUploadMessage(uploadingPhoto);
	...
	const handleTakePhotoClick = (index: number) => {
		console.log("Capturando foto para o membro no índice:", index);
		localStorage.setItem("takePhotoClicked", "true");
		localStorage.setItem("memberIndex", index.toString());
		navigate("/capturar");
	};

	const handleTakePhotoClickNoAI = (index: number) => {
		console.log("Capturando foto (sem IA) para o membro no índice:", index);
		localStorage.setItem("takePhotoClicked", "true");
		localStorage.setItem("memberIndex", index.toString());
		navigate("/capturarSemIa");
	};

	const isUpdating = useRef<{ [key: number]: boolean }>({});

	useEffect(() => {
		const handlePhotoUploadStarted = () => {
			setUploadingPhoto(true);
			document.querySelector(".popupBackgroundBlur")?.classList.add("opacityControl");
			console.log("Foto está sendo carregada...");
		};

		const handlePhotoUploadFinished = () => {
			setUploadingPhoto(false);
			document.querySelector(".popupBackgroundBlur")?.classList.remove("opacityControl");
			console.log("Carregamento de foto concluído.");
		};

		window.addEventListener("photoUploadStarted", handlePhotoUploadStarted);
		window.addEventListener("photoUploadFinished", handlePhotoUploadFinished);

		return () => {
			window.removeEventListener("photoUploadStarted", handlePhotoUploadStarted);
			window.removeEventListener("photoUploadFinished", handlePhotoUploadFinished);
		};
	}, []);

	// Upload de foto por captura
	const handleUpdateImage = useCallback(async (index: number, memberId: string, newImageUrl: string) => {
		if (isUpdating.current[index]) {
			console.log(`Já está atualizando a imagem do membro no índice: ${index}`);
			return;
		}

		isUpdating.current[index] = true;
		console.log(`Atualizando imagem do membro no índice: ${index}`);
		...
		try {
			...
			const response = await cadasterMemberPhoto(memberId, blob);
			...
			if (response.isFailure) {
				if (response.error === "Not Found") {
					...
					faultMessages.showError("Erro na selfie", response.error);
				}
			} else {
			...
			console.error(error);
			faultMessages.showError("Isso foi... Inesperado", "Tente recarregar a página e tirar outra foto, isso deve arrumar esse problema que tivemos.");
		} finally {
	...
	useEffect(() => {
		const fetchMembers = async () => {
			if (!hasFetchedMembers) {
				try {
					...
					if (capturedImage && memberIndex !== null) {
						const index = parseInt(memberIndex, 10);
						if (index >= 0 && index < membersData.length) {
							console.log("Atualizando a imagem para o membro:", index);
							await handleUpdateImage(index, membersData[index].memberId, capturedImage);
							handleUpdateHasPhoto(index, true);
						} else {
							console.error("Erro: índice de membro inválido:", memberIndex);
						}
		...
		fetchMembers();
	}, [titleCode, handleUpdateImage, hasFetchedMembers]);

	const handleEventUpdate = useCallback(
		(event: CustomEvent<{ index: number; memberId: string; newImageUrl: string }>) => {
			console.log("Evento handleUpdate recebido");
			const { index, memberId, newImageUrl } = event.detail;
			handleUpdateImage(index, memberId, newImageUrl);
			handleUpdateHasPhoto(index, true);
		},
		[handleUpdateImage]
	);

	const handleUpdateHasPhoto = (index: number, hasPhoto: boolean) => {
		setMembers((prevMembers) => {
			const updatedMembers = [...prevMembers];
			updatedMembers[index] = { ...updatedMembers[index], hasPhoto };
			return updatedMembers;
		});
	};

	useEffect(() => {
		window.addEventListener("handleUpdate", handleEventUpdate as EventListener);
		return () => {
			window.removeEventListener("handleUpdate", handleEventUpdate as EventListener);
		};
	}, [handleEventUpdate]);
		...
		<div>
				...
				{uploadingPhoto && (
					<div className="popupWrapper">
						<div className="recoginitionPopup">
							...
							<div className={`sendingMessageUpload ${!isTransitioning ? "show" : ""}`}>
								<h5>{sendingMessage.title}</h5>
								<p>{sendingMessage.description}</p>
							</div>
						</div>
					</div>
				)}
			...
			<div className="homeBody">
				...
				<div className="homeWrapper">
					<div className="homeWrapperHeader">
						<div className="homeWrapperHeaderContent">
							<svg className="cameraIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
								<path
									...
									fill="var(--wrapper-icon-color)"
								/>
							</svg>
							<h1 className="homeHeaderTitle">Biometria facial</h1>
							<span className="homeHeaderLine"></span>
						</div>
					</div>
					<div className="homeMemberTitleInfo">
						<h2>Título: {titleCode?.toUpperCase()}</h2>
					</div>
					<div className="homeWrapperSection">
						<div className="homeWrapperCards">
							{loading
								? Array(2)
									.fill(0)
									.map((_, index) => <MemberCardSkeleton key={index} />)
								: members.map((member, index) => (
									<div key={index} id={`MemberCard-${index}`} className="cardContainer">
										<MemberCard
											key={member.memberId}
											member={member}
											name={member.name}
											...
											onTakePhotoClickNoAI={handleTakePhotoClickNoAI}
											onUpdateImage={(newImageUrl) => handleUpdateImage(index, member.memberId, newImageUrl)}
											onUpdateHasPhoto={handleUpdateHasPhoto}
										/>
									</div>
								))
							}
